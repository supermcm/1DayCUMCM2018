...
if nargin==0
    proj = load('data/3.dat');              % phase/initial angle
    phi = 30;
...
img = iradon(proj,[0:179]+phi,'Hann');

% cut out the right region: x in [xc-50, xc+50]; y in [yc-50 yc+50]
nhalf = size(img,1)/2;
xidx = ceil(-xc/d) + nhalf + [ceil(-50/d):ceil(50/d)];
yidx = ceil( yc/d) + nhalf + [ceil(-50/d):ceil(50/d)];

img = img(yidx,xidx);

% changle img from 362x362 to 256x256
img = imrescale(img, [256,256]);
imgud = flipud(img);
...
% show the image
imagesc([0,100], [0,100], imgud); colorbar
hold on; axis image
% plot the 10 points: absorption values ('x'indicates =0, '+' indicates >0)
plot(xy(imgud(idx)==0,1), xy(imgud(idx)==0,2), 'wx', 'linewidth', 2)
plot(xy(imgud(idx)~=0,1), xy(imgud(idx)~=0,2), 'r+', 'linewidth', 2)
